...
1. 打开首页。
2. 点击 **进入酒馆应用**，进入 `tavern.html`。
3. 根据需要修改全局场景设定。
4. 设置对话轮数和发言间隔。
5. 在右侧配置智能体：主卡片只显示名称、人格提示词和启用状态；点击小齿轮可设置模型名、temperature、max_tokens。
6. 如需新增角色，可使用“手动添加智能体”表单填写名称和人格提示词；新增角色的模型参数同样收纳在表单右上角小齿轮中，也可点击“添加智能体”快速生成默认新客人。
7. 点击 **开始对话**。
8. 多个智能体会按顺序轮流发言。
9. 下方模拟终端会同步显示请求日志和 AI 输出。
10. 如需中断，点击 **停止对话**。
11. 如需重置聊天区，点击 **清空聊天**。
