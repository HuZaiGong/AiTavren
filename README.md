...
在线访问首页：<https://huzaigong.github.io/AiTavren/>

场景生成页面：<https://huzaigong.github.io/AiTavren/setup.html>

酒馆应用页面：<https://huzaigong.github.io/AiTavren/tavern.html>

## 功能特性

- **纯前端静态应用**：HTML、CSS、JavaScript 分离，无需后端、无需 Node.js、无需构建步骤。
- **三段式体验**：首页介绍项目，中间页生成/选择酒馆设定，应用页进行多智能体对话。
- **GPT-5.5 场景生成页**：可根据用户描述生成酒馆设定，也可一键推荐多个设定；用户可以跳过或选择设定后直接对话。
...
- **可配置模型参数**：每个智能体可单独设置名称、人格提示词、是否启用；模型名、temperature、max_tokens 收纳在小齿轮菜单中，不占用主页面空间。
- **全局场景设定**：可编辑酒馆背景，例如边境小镇、雨夜、北境来信等；从生成页选择的设定会自动带入应用页。
...
- **响应式界面**：首页、生成页、应用页均加强了移动端适配，按钮、卡片和表单更适合手机触控。
...
1. 打开首页。
2. 点击 **先生成酒馆设定**，进入 `setup.html`。
3. 在生成页输入自己的酒馆描述，或点击 **推荐几个设定**。
4. 可以选择某个生成设定并进入对话，也可以跳过生成页直接进入 `tavern.html`。
5. 进入应用页后，所选设定会自动填入“全局场景设定”；也可以继续手动修改。
6. 设置对话轮数和发言间隔。
7. 在右侧配置智能体：主卡片只显示名称、人格提示词和启用状态；点击小齿轮可设置模型名、temperature、max_tokens。
8. 如需新增角色，可使用“手动添加智能体”表单填写名称和人格提示词；新增角色的模型参数同样收纳在表单右上角小齿轮中，也可点击“添加智能体”快速生成默认新客人。
9. 点击 **开始对话**。
10. 多个智能体会按顺序轮流发言。
11. 下方模拟终端会同步显示请求日志和 AI 输出。
12. 如需中断，点击 **停止对话**。
13. 如需重置聊天区，点击 **清空聊天**。
...
AiTavren/
├── AAA.md      # 原始需求文档
├── index.html   # 网站首页 / landing page
├── home.css     # 首页样式
├── setup.html   # 酒馆设定生成/选择中间页
├── setup.css    # 中间页样式
├── setup.js     # 中间页 GPT-5.5 调用和设定传递逻辑
├── tavern.html  # 酒馆应用页面
├── styles.css   # 酒馆应用样式
├── app.js       # 酒馆应用逻辑
└── README.md    # 项目说明
